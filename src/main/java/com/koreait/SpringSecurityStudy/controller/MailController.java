...
import com.koreait.SpringSecurityStudy.security.jwt.model.PrincipalUser;
...
public class MailController {
    ...
    @PostMapping("/send")
    private ResponseEntity<?> sendMail(@RequestBody SendMailReqDto sendMailReqDto, @AuthenticationPrincipal PrincipalUser principalUser){
        return ResponseEntity.ok(mailService.sendMail(sendMailReqDto, principalUser));
    }
    @GetMapping("/verify")
    private String verify(Model model, @RequestParam String verifyToken){
